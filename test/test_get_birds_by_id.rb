...
# Test cases for 'GET /birds/{id}' endpoint
class TestGetBirdsByID < Minitest::Test
        def setup
	...
	def test_get_birds_by_id_visible_set_to_true
		base_url = "http://localhost:7777/birds"
		header = {"Content-Type" => "application/json"}
		json_data = '{"name": "Snow partridge", "family": "Lerwa lerwa", "continents": ["India"], "visible": true}'
		response = @httpclint.post(base_url, json_data, header)
		status_code = response.status
		# check status code
		assert_equal(201, status_code)	
